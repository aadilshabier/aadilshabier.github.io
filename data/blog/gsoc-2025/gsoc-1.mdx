...
## What is swhkd?
Short for Simple Wayland HotKey Daemon. I'll borrow the answer from the [website](https://git.sr.ht/~shinyzenith/swhkd#swhkd)
...
proposal. In the first phase of this project, our goals are:
...
`AsyncFd`[^1] struct from `tokio`, which provides a wrapper around file descriptors to
facilitate asynchronous reads and writes.

[^1]: AsyncFD: https://docs.rs/tokio/latest/tokio/io/unix/struct.AsyncFd.html
...
`poll` and `epoll`. There are also several blogs and resources[^2] on the internet to learn more
about how this is used in practice.  

[^2]: Chapter 6. I/O Multiplexing: The select and poll Functions: https://notes.shichao.io/unp/ch6/

Libinput opens devices in non-blocking mode by default, allowing us to take advantage of it. We 
pair it with Tokio's AsyncFd[^1], 
